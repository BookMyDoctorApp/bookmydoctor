...
public class HospitalController {
	...
	@GetMapping("allDoctors/id/{hospitalId}")
	public List<Doctor> getDoctorsByHospital(@PathVariable int hospitalId) {

		Hospital savedHospital = iHospitalService.getHospitalById(hospitalId);
		return savedHospital.getDoctors();

	}
	
	@GetMapping("/id/{hospitalId}/speciality/{speciality}")
	public List<Doctor> getDoctorsByHospitalAndSpeciality(@PathVariable int hospitalId, @PathVariable String speciality) {

		Hospital savedHospital = iHospitalService.getHospitalById(hospitalId);
		List<Doctor> outputList=new ArrayList<>();
		if(savedHospital!=null) {
			List<Doctor> allDoctors = savedHospital.getDoctors();
			for (Doctor doctor : allDoctors) {
				if(doctor.getSpeciality().equalsIgnoreCase(speciality)) {
					outputList.add(doctor);
				}
			}
