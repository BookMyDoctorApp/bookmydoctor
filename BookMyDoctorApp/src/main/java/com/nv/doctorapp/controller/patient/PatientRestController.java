...
public class PatientRestController {
	...
	@PutMapping("/{patientId}/hospital/{hospitalId}")
	public ResponseEntity<PatientResponseDTO>updatePatientByHospitalId(@PathVariable int patientId,
			@PathVariable int hospitalId) {
		Patient updatedPatient = patientService.updatePatientByHospitalId(patientId, hospitalId);
	...
	/*
	@GetMapping("/listOfPatient/{hospitalId}")
	public ResponseEntity<List<PatientResponseDTO>> getPatientByHospitalId(@PathVariable int hospitalId)
	{
		List<Patient> allPatients = patientService.getPatientByHospitalId(hospitalId);
		List<PatientResponseDTO> dtoObj = new ArrayList<>();
		for (Patient patient : allPatients) 
			dtoObj.add(dtoConvertor.convertTo(patient));
		return new ResponseEntity<List<PatientResponseDTO>>(dtoObj, HttpStatus.OK);
